...
private:
  ...
  ros::Subscriber subSetVelocitiesAccelerationsRaw_;
  ...
  std::vector<double> accelerations_;  // in rad/s^2
  std::vector<double> velocities_and_accelerations_;  // in rad/s
  bool hasNewGoal_;
...
public:
  ...
  bool init()
  {
    ...
    subSetVelocitiesAccelerationsRaw_ = nh_.subscribe("joint_group_velocity_acceleration_controller/command", 1,
                                                      &SdhNode::topicCallback_setVelocitiesAccelerationsRaw, this); // TODO: Fix this
    ...
    accelerations_.resize(DOF_);
    velocities_and_accelerations_.resize(2*DOF_);
    for (int i = 0; i < DOF_; i++)
    {
      axes_[i] = i;
    }
  ...
  bool switchOperationMode(const std::string &mode)
  {
    ...
    try
    {
      ...
      else if (mode == "velocity_acceleration")
      {
        sdh_->SetController(SDH::cSDH::eCT_VELOCITY_ACCELERATION);
      }
      else
      {
  ...
  void topicCallback_setVelocitiesAccelerationsRaw(const std_msgs::Float64MultiArrayPtr& velocities_and_accelerations)
  {
	  //TODO implement this
    if (!isInitialized_)
    {
    ...
    if (velocities_and_accelerations->data.size() != velocities_and_accelerations_.size())
    {
      ROS_ERROR("Velocity and acceleration array dimension mismatch");
      return;
    }
    if (operationMode_ != "velocity_acceleration")
    {
      ROS_ERROR("%s: Rejected, sdh not in velocity_acceleration mode", action_name_.c_str());
      return;
    }
    while (hasNewGoal_ == true)
      usleep(10000);

    velocities_[0] = velocities_and_accelerations->data[0] * 180.0 / pi_;  // sdh_knuckle_joint
    velocities_[1] = velocities_and_accelerations->data[5] * 180.0 / pi_;  // sdh_finger22_joint
    velocities_[2] = velocities_and_accelerations->data[6] * 180.0 / pi_;  // sdh_finger23_joint
    velocities_[3] = velocities_and_accelerations->data[1] * 180.0 / pi_;  // sdh_thumb2_joint
    velocities_[4] = velocities_and_accelerations->data[2] * 180.0 / pi_;  // sdh_thumb3_joint
    velocities_[5] = velocities_and_accelerations->data[3] * 180.0 / pi_;  // sdh_finger12_joint
    velocities_[6] = velocities_and_accelerations->data[4] * 180.0 / pi_;  // sdh_finger13_joint
    accelerations_[0] = velocities_and_accelerations->data[7] * 180.0 / pi_;  // sdh_finger22_joint
    accelerations_[1] = velocities_and_accelerations->data[12] * 180.0 / pi_;  // sdh_finger22_joint
    accelerations_[2] = velocities_and_accelerations->data[13] * 180.0 / pi_;  // sdh_finger23_joint
    accelerations_[3] = velocities_and_accelerations->data[8] * 180.0 / pi_;  // sdh_thumb2_joint
    accelerations_[4] = velocities_and_accelerations->data[9] * 180.0 / pi_;  // sdh_thumb3_joint
    accelerations_[5] = velocities_and_accelerations->data[10] * 180.0 / pi_;  // sdh_finger12_joint
    accelerations_[6] = velocities_and_accelerations->data[11] * 180.0 / pi_;  // sdh_finger13_joint
    hasNewGoal_ = true;
  }
  ...
  bool srvCallback_Init(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
  {
    if (isInitialized_ == false)
    {
      ...
      try
      {
        ...
        max_accelerations_ = sdh_->GetAxisMaxAcceleration( sdh_->all_real_axes );
        for (int i = 0; i<DOF_; i++)
        {
          ROS_INFO("Max acceleration %d = %f", i, max_accelerations_[i]);
        }
      }
      catch (SDH::cSDHLibraryException* e)
      {
  ...
  bool srvCallback_SetOperationMode(cob_srvs::SetString::Request &req, cob_srvs::SetString::Response &res)
  {
    ...
    else if (operationMode_ == "velocity_acceleration")
    {
    	try
    	{
        sdh_->SetController(SDH::cSDH::eCT_VELOCITY_ACCELERATION);
  ...
  void clampAccelerations()
  {
	  for (int i=0; i< max_accelerations_.size(); i++){

		  accelerations_[i] = SDH::ToRange(accelerations_[i], -max_accelerations_[i], max_accelerations_[i]);
	  }
  ...
  void updateSdh()
  {
    ...
    if (isInitialized_ == true)
    {
      if (hasNewGoal_ == true)
      {
        ...
        {
        	// TODO write a code using void cSDH::SetAxisTargetAcceleration( std::vector<int>const& axes, std::vector<double>const& accelerations )
        	// Similar to as in block else if (operationMode_ == "velocity")
          try
          {
            clampAccelerations();
            sdh_->SetAxisTargetAcceleration(axes_, accelerations_);
            clampVelocities();
            sdh_->SetAxisTargetVelocity(axes_, velocities_);
            // ROS_DEBUG_STREAM("velocities: " << velocities_[0] << " "<< velocities_[1] << " "<< velocities_[2] << " "<< velocities_[3] << " "<< velocities_[4] << " "<< velocities_[5] << " "<< velocities_[6]);
          }
        ...
        {
          ROS_ERROR("sdh neither in position nor in velocity nor in velocity_acceleration nor in effort mode. OperationMode = [%s]",
